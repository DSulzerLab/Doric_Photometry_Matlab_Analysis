function [ZScoreTrace_NoNaN] = CorrelationErrorBarPlot(Time,movement_trace,gcamp_trace)
...
while i<=size(ZScoreTrace_NoNaN,2)
    alighned_pearson_R=[alighned_pearson_R ZScoreTrace_NoNaN(size(movement_trace),i)];
    i=i+1;
end
