function [ZScoreTrace] = C4_Convert_shock_to_Zscore(Time,trace)
%this will take a set of tones from 1 mouse on 1 day 

%then will smooth the traces
smooth_traces=movmean(trace,100);
...
for i=1:n
    preCS_means(i)=mean(z_traces(1:488,i),1);
end
...
AUC_average_trace=C4_ShockAnalyzeAUC(ZScoreTrace);
AverageZ_average_trace=C4_ShockAnalyzeAverageZ(ZScoreTrace);
MaxAmp_average_trace=C4_ShockAnalyzeMaxAmp(ZScoreTrace);
